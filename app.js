...
// Bidireccional y SIN bloqueo de scroll. #pour-scroll mide 230vh; su hijo
// .pour-stage queda sticky mientras se recorre. El progreso se calcula así:
//     progreso = (-rect.top) / (rect.height - alto de la ventana)   [0..1]
// Al bajar el progreso sube (se llena: arte-1 -> arte-4); al subir baja
// (se vacía). Un pequeño suavizado por requestAnimationFrame (equivale a
// scrub ~0.5) evita saltos con scroll brusco; no hay layout thrashing porque
// solo se leen medidas en el evento de scroll y se escriben estilos en rAF.
function initPourScene() {
  const section = document.getElementById("pour-scroll");
  if (!section) return;
  const stage = document.getElementById("pour-stage");
  const photos = Array.from(section.querySelectorAll(".pour-photo"));
  const bar = document.getElementById("pour-progress-bar");
  if (!stage || photos.length < 2) return;

  const N = photos.length;             // nº de fotos (arte-1..arte-4)
  const SMOOTH = 0.18;                  // 0..1 — más bajo = más suave
  const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

  let current = 0, target = 0, rafId = null;

  // Lee la posición del scroll y calcula el progreso objetivo (0..1).
  function readScroll() {
  ...
  // Dibuja el estado: funde las fotos y mueve la barra de progreso.
  function paint(p) {
    // posición del "cabezal" entre 0 y N-1
    const head = p * (N - 1);
    photos.forEach((img, i) => {
      // opacidad triangular: 1 en el centro de la foto i, 0 en las vecinas
      let o = 1 - Math.abs(head - i);
      o = o < 0 ? 0 : o;
      // la primera y la última se mantienen visibles en los extremos
      if (i === 0 && head <= 0) o = 1;
      if (i === N - 1 && head >= N - 1) o = 1;
